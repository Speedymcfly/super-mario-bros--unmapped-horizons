...
func _on_collect_body_entered(body: Node2D) -> void:
	if body is player:
		AudioManager.play_sfx_2(load("res://assets/audio/SFX/1up.wav"))
		...
		if Globals.shared_lives == true:
			Globals.lives += 1
		if (Globals.mario_lives < 99 or Globals.luigi_lives < 99 or Globals.toad_lives < 99 or Globals.toadette_lives < 100 or Globals.peach_lives < 100 or Globals.daisy_lives < 100 and Globals.shared_lives == false) or Globals.lives < 99 and Globals.shared_lives == true:
