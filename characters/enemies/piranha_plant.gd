...
@export_enum(
	...
	"Green_Yellow",
	"Red_White",
	"Red_White_Green_Lips",
	"Red_White_Yellow_Teeth",
	"Blue",
	"Yellow",
	"Purple",
	"Gold"
	) var head_colour = "Green"
@export_enum(
	"Green",
	"Blue",
	"Light_Blue",
...
func _ready() -> void:
	var head_frames = load("res://characters/enemies/Pakkun_Head_" + str(head_colour) + ".tres")
	head.sprite_frames = head_frames
	var stem_frames = load("res://characters/enemies/Pakkun_Stem_" + str(stem_size) + "_" + str(stem_colour) + ".tres")
...
func _on_in_out_timer_timeout() -> void:
	match pipe_state:
		...
		Pipestate.Out:
			...
			start_pipe_cycle()
func is_green_piranha() -> bool:
	return head_colour.begins_with("Green")
func freeze():
	frozen = true
	add_to_group("frozen")
